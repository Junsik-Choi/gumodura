...
export default function VatCalculator() {
  ...
  const [
    ...
    '부가세율 10% 적용',
    '간이과세자:',
    '업종별 1.5%~4% 적용 (별도 계산 필요)',
    '면세사업자:',
    '부가세 면제 (의료, 교육 등)',
    '신고 기간:',
    '1기(1~6월) → 7/25까지, 2기(7~12월) → 다음해 1/25까지',
    '금액을 입력하면',
    '부가세가 자동으로 계산됩니다.',
  ]);
  ...
  const quickAmounts = [
    { label: 일만원, value: 10000 },
    { label: 오만원, value: 50000 },
    { label: 십만원, value: 100000 },
    { label: 오십만원, value: 500000 },
    { label: 백만원, value: 1000000 },
    { label: 오백만원, value: 5000000 },
  ];

  const getPlaceholder = () => {
    switch (mode) {
      case 'fromTotal': return 합계금액부가세포함;
      case 'fromSupply': return 공급가액부가세미포함;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <p className="font-semibold text-gray-700">📊 {계산방식선택}</p>
      ...
      {/* 세율 선택 */}
      <div className="space-y-2">
        <p className="font-semibold text-gray-700">📋 {세율선택}</p>
      ...
      {parseFloat(inputValue) > 0 && (
        <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl p-6 border-2 border-emerald-200">
          <h3 className="font-bold text-gray-800 mb-4 text-center">📝 {계산결과}</h3>
            ...
            <div className={`bg-white rounded-xl p-4 ${mode === 'fromSupply' ? 'ring-2 ring-ai-primary' : ''}`}>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-xs text-gray-400">{부가세미포함}</p>
                </div>
            ...
            <div className={`bg-white rounded-xl p-4 border-2 border-emerald-300 ${mode === 'fromTotal' ? 'ring-2 ring-ai-primary' : ''}`}>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-xs text-gray-400">{부가세포함}</p>
                </div>
          ...
          <div className="mt-4 pt-4 border-t border-emerald-200">
            <p className="text-sm text-gray-600 text-center">
              ...
              {mode === 'fromSupply' && (
                <>{부가세액} = {formatMoney(parseFloat(inputValue) || 0)} × {taxRate}% = <span className="font-medium">{formatMoney(calculation.taxAmount)}</span>{원}</>
              )}
              {mode === 'fromTax' && (
      ...
      <div className="bg-gray-50 rounded-xl p-4">
        <h3 className="font-bold text-gray-700 mb-3">💡 {부가세참고정보}</h3>
        <div className="space-y-2 text-sm text-gray-600">
          <p>• <span className="font-medium">{일반과세자}</span> {부가세율10적용}</p>
          <p>• <span className="font-medium">{간이과세자}</span> {업종별적용}</p>
          <p>• <span className="font-medium">{면세사업자}</span> {부가세면제}</p>
          <p>• <span className="font-medium">{신고기간}</span> {신고기간내용}</p>
        </div>
      </div>
      ...
      {!parseFloat(inputValue) && (
        <div className="bg-gray-50 rounded-2xl p-8 text-center">
          ...
          <p className="text-gray-500">{금액입력안내1}<br/>{금액입력안내2}</p>
        </div>
      )}
    </div>
  );
