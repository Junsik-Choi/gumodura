...
const QUALITY_PRESETS = [
  { name: '최고 품질', quality: 95, icon: '💎', desc: '거의 원본' },
  { name: '고품질', quality: 85, icon: '✨', desc: '추천' },
  { name: '중간', quality: 70, icon: '⚖️', desc: '균형잡힌' },
  { name: '작은 용량', quality: 50, icon: '📦', desc: '용량 우선' },
];
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-sm text-gray-500">여러 장 선택 가능</p>
        <input
          ref={fileInputRef}
          type="file"
          accept="image/*"
      ...
      {images.length > 0 && (
        <>
          ...
          {/* 품질 프리셋 */}
          <div>
            <p className="font-semibold text-gray-700 mb-3">✨ 압축 품질</p>
          ...
          >
            {processing ? '압축 중...' : `📉 ${images.length}개 이미지 압축하기`}
          </button>
          ...
          {compressed && totalCompressed > 0 && (
            <div className="bg-green-50 border-2 border-green-200 rounded-xl p-6 text-center space-y-4">
              ...
              <p className="text-xl font-bold text-green-700">압축 완료!</p>
